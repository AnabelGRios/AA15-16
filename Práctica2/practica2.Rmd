...
while (N <= 118) {
  ...
  for(j in 1:N) {
    ...
    valor_real <- crossprod(wf,xn)
    valor_estimado <- crossprod(wreg,xn)
    e_n <- (valor_real - valor_estimado)^2
    e[j] <- e_n
  }
  errores[[i]] <- e
...
```{r}
print("La media de e1, e2 y Ecv ha sido, para cada N y por orden:")
print(errores)
print("La varianza de e1, e2 y Ecv ha sido, para cada N y por orden:")
print(varianzas)
```
...
```
Como vemos, la diferencia entre el promedio de los valores de $e_1$, $e_2$ y $E_{cv}$ es muy pequeña (y se va haciendo incluso más pequeña cuanto mayor es la $N$), que es algo que tiene sentido ya que lo que aquí se está representando son las medias de los 1000 valores que nos han salido de $e_1$, $e_2$ y $E_{cv}$ para cada valor de $N$. Es lógico que las medias de $e_1$ y $e_2$ se parezcan si pensamos en lo que estamos haciendo: para cada valor en la muestra, lo sacamos de la misma y calculamos una función que ajuste los demás, y $e_i$ es el valor de error cuadrático que tiene el valor que hemos sacado fuera. Puede que algún valor esté muy lejos de la función estimada o que alguno caiga dentro, pero en media quedarán más o menos todos a la misma distancia, sin importar que el valor que hemos sacado sea el primero, el segundo o el último, por lo que las medias de $e_i$ serán parecidas y, como es lógico, cuanto mayor sea el número de datos en la muestra ($N$), mejor estimaremos la función y menor error habrá, como se puede apreciar en la gráfica. Habiendo visto que todos los $e_i$, al ser medias, se parecerán, es lógico que se parezcan también a $E_{cv}$, ya que es la media de todos ellos.
...
Lo que contribuye a la varianza tanto de $e_1$ como de $e_2$ y $E_{cv}$ son los valores que se quedan lejos de la media. Como vemos en la gráfica, las varianzas de $e_1$ y $e_2$ están más o menos cercanas (excepto para $N=18$) y cada vez se van acercando más a la varianza de $E_{cv}$ cuando mayor es $N$. Esto también es lógico. Vamos a centrarnos primero en valores pequeños de $N$: los valores que se quedan lejos de la media influyen más porque tenemos pocos datos. Puede darse que haya más valores que se quedan lejos de la media y tienen más error, como es el caso de $e_1$ o que haya menos, como le pasa a $e_2$, pero si cogemos la varianza de la media de todos los $e_i$ será menor que la de todos ellos, puesto que es la media de todos. Además, cuanto mayor sea $N$, más datos habrá, mejor será el ajuste y menor será el error de un dato frente a todos los demás y por tanto menor será la varianza de ese dato. Si de nuevo hacemos la media, la varianza de $e_1$ y $e_2$ bajará también.
...
N <- seq(18, 118, by=10)
Neff <- 100*(e_1/Ecv)/N
pinta_particion(N, Neff, rep(1,11), main="Neff", xlab="Valor de N", ylab="Neff")
```
